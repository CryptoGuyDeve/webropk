...
export default function ShopifyStoreManagingClient({ data }: { data: ServiceData | null }) {
  ...
  const DEFAULT_PACKAGES = [
    {
      ...
      features: ["Weekly Health Check", "Link Monitoring", "Basic Product Updates (5/mo)", "Monthly Report"],
    },
    {
      name: "Growth",
      price: "$500",
      description: "Daily operations handling.",
      features: ["Daily Order Fulfillment", "Inventory Sync", "Product Updates (20/mo)", "Conversion Audit", "Speed Optimization"],
      isPopular: true
    },
    {
      name: "Enterprise",
      price: "$1000",
      description: "Full automation & scaling.",
      features: ["Dedicated Manager", "Unlimited Product Updates", "App Configuration", "Advanced Reporting", "24/7 Priority Support", "Supplier Coordination"],
    }
  ];
          ...
          <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-20">
            ...
            >
              ...
              <p className="text-lg md:text-xl text-muted-foreground mb-8 leading-relaxed max-w-2xl mx-auto lg:mx-0">
                {data?.heroSubtitle || "Stop worrying about inventory, order fulfillment, and tech issues. We handle the day-to-day operations so you can focus on profits."}
              </p>
            ...
            {packages.map((pkg: any, i: number) => {
              ...
              const CardWrapper = isPopular ?
                ({ children }: { children: React.ReactNode }) => (
                  <Card className="border-green-400 shadow-xl relative scale-105 z-10 bg-white dark:bg-zinc-900 border-2">
              ...
              const titleColor = isPopular ? "text-green-600" : "";
              ...
              return (
                <CardWrapper key={i}>
                    ...
                    <ul className="space-y-3">
                      {pkg.features?.map((f: string, j: number) => (
                        <li key={j} className="flex items-center gap-2 text-sm text-foreground">
                          <CheckCircle2 className={`w-4 h-4 ${isPopular ? "text-green-500" : "text-gray-400"}`} /> {f}
                        </li>
                      ))}
                    </ul>
                  </CardContent>
                  <CardFooter>
                    <Button
                      className={`w-full ${isPopular ? "bg-green-500 hover:bg-green-600 text-white" : ""}`}
                      variant={isPopular ? "default" : "outline"}
                      onClick={() => openIntercomWithMessage(`Hi, I'm interested in the ${pkg.name} Management Plan (${pkg.price}).`)}
                    >
                      Select {pkg.name}
                    </Button>
                  </CardFooter>
