...
export default function ShopifyStoreCreationClient({ data }: { data: ServiceData | null }) {
  ...
  const DEFAULT_PACKAGES = [
    {
      name: "Basic",
      price: "$100",
      description: "Perfect for testing a new product.",
      features: ["Shopify Setup", "Free Theme Customization", "Upload 5 Products", "Basic SEO", "Mobile Responsive"],
    },
    {
      name: "Standard",
      price: "$250",
      description: "For serious brands ready to scale.",
      features: ["Everything in Basic", "Premium Theme Setup", "Upload 20 Products", "Advanced SEO", "Payment Gateway Setup", "5 Plugins Configured", "Social Media Integration"],
      isPopular: true
    },
    {
      name: "Premium",
      price: "$500",
      description: "Full-scale automated dropshipping store.",
      features: ["Everything in Standard", "Upload 50+ Products", "Logo Design Included", "Win Product Research", "Email Marketing Setup", "Dropshipping Automation", "1 Month Support"],
    }
  ];
          ...
          <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-20">
            ...
            >
              ...
              <p className="text-lg md:text-xl text-muted-foreground mb-8 leading-relaxed max-w-2xl mx-auto lg:mx-0">
                {data?.heroSubtitle || "Launch your brand with a premium, speed-optimized, and fully automated Shopify store designed to turn visitors into loyal customers."}
              </p>
        ...
        <div className="container px-4 mx-auto">
          ...
          <div className="grid lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {packages.map((pkg: any, i: number) => {
              const isPopular = pkg.isPopular || (i === 1 && !data?.packages); // Default logic: 2nd item is popular if using default data
              const CardWrapper = isPopular ?
                ({ children }: { children: React.ReactNode }) => (
                  <Card className="border-yellow-400 shadow-xl relative scale-105 z-10 bg-white dark:bg-zinc-900 border-2">
              ...
              const titleColor = isPopular ? "text-yellow-600" : "";
              ...
              return (
                <CardWrapper key={i}>
                    ...
                    <ul className="space-y-3">
                      {pkg.features?.map((f: string, j: number) => (
                        <li key={j} className="flex items-center gap-2 text-sm">
                          <CheckCircle2 className={`w-4 h-4 ${isPopular ? "text-yellow-500" : "text-gray-400"}`} /> {f}
                        </li>
                      ))}
                    </ul>
                  </CardContent>
                  <CardFooter>
                    <Button
                      className={`w-full ${isPopular ? "bg-yellow-500 hover:bg-yellow-600 text-white" : ""}`}
                      variant={isPopular ? "default" : "outline"}
                      onClick={() => openIntercomWithMessage(`Hi, I'm interested in the ${pkg.name} Package (${pkg.price}).`)}
                    >
                      Select {pkg.name}
                    </Button>
                  </CardFooter>
