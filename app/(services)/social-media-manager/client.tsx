...
export default function SocialMediaManagerPageClient({ data }: { data: ServiceData | null }) {
  ...
  const DEFAULT_PACKAGES = [
    {
      ...
      features: ["3 Posts per Week", "Caption Writing", "Hashtag Research", "Monthly Content Calendar", "1 Platform"],
    },
    {
      name: "Active",
      price: "$450",
      description: "Building real growth.",
      features: ["5 Posts per Week", "2 Platforms (e.g. FB & IG)", "Story Updates (3/week)", "Community Management", "Monthly Strategy Call"],
      isPopular: true
    },
    {
      name: "Influencer",
      ...
      features: ["Daily Posting (7/week)", "3 Platforms", "Reels/TikTok Editing", "Influencer Outreach", "Priority Support", "Quarterly Analytics Deep Dive"],
    }
  ];
          ...
          <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-20">
            ...
            >
              ...
              <p className="text-lg md:text-xl text-muted-foreground mb-8 leading-relaxed max-w-2xl mx-auto lg:mx-0">
                {data?.heroSubtitle || "Consistent content, engaging captions, and real community building. We turn your social media followers into loyal fans."}
              </p>
          ...
          <div className="grid lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
            <div className="grid lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
              {packages.map((pkg: any, i: number) => {
                ...
                const titleColor = isPopular ? "text-purple-600" : "";
                ...
                return (
                  <CardWrapper key={i}>
                      ...
                      <ul className="space-y-3">
                        {pkg.features?.map((f: string, j: number) => (
                          <li key={j} className="flex items-center gap-2 text-sm text-foreground">
                            <CheckCircle2 className={`w-4 h-4 ${isPopular ? "text-purple-500" : "text-gray-400"}`} /> {f}
                          </li>
                        ))}
                      </ul>
                    </CardContent>
                    <CardFooter>
                      <Button
                        className={`w-full ${isPopular ? "bg-purple-500 hover:bg-purple-600 text-white" : ""}`}
                        variant={isPopular ? "default" : "outline"}
                        onClick={() => openIntercomWithMessage(`Hi, I'm interested in the ${pkg.name} Social Media Package (${pkg.price}).`)}
                      >
                        Select {pkg.name}
                      </Button>
                    </CardFooter>
