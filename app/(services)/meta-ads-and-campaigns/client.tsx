...
export default function MetaAdsPageClient({ data }: { data: ServiceData | null }) {
  ...
  const DEFAULT_PACKAGES = [
    {
      name: "Tester",
      price: "$300",
      description: "Single campaign testing.",
      features: ["1 Campaign Setup", "3 Ad Sets", "Audience Research", "Monthly Report", "Pixel Check"],
    },
    {
      name: "Scaler",
      price: "$600",
      description: "Multi-campaign growth.",
      features: ["3 Campaigns (Cold/Retargeting)", "10 Ad Sets", "Weekly Optimization", "Creative Recommendations", "Custom Reporting"],
      isPopular: true
    },
    {
      name: "Dominator",
      ...
      features: ["Unlimited Campaigns", "CAPI Setup (Server-side)", "Dedicated Strategist", "Copywriting Included", "Bi-Weekly Calls", "Cross-Platform Strategy"],
    }
  ];
          ...
          <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-20">
            ...
            >
              ...
              <p className="text-lg md:text-xl text-muted-foreground mb-8 leading-relaxed max-w-2xl mx-auto lg:mx-0">
                {data?.heroSubtitle || "Stop burning budget on guessing. We build data-driven Facebook & Instagram campaigns that scale your ROI and sales."}
              </p>
        ...
        <div className="container px-4 mx-auto">
          ...
          <div className="grid lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {packages.map((pkg: any, i: number) => {
              ...
              const titleColor = isPopular ? "text-blue-600" : "";
                ...
                <CardWrapper key={i}>
                    ...
                    <ul className="space-y-3">
                      {pkg.features?.map((f: string, j: number) => (
                        <li key={j} className="flex items-center gap-2 text-sm text-foreground">
                          <CheckCircle2 className={`w-4 h-4 ${isPopular ? "text-blue-600" : "text-gray-400"}`} /> {f}
                        </li>
                      ))}
                    </ul>
                  </CardContent>
                  <CardFooter>
                    <Button
                      ...
                      onClick={() => openIntercomWithMessage(`Hi, I'm interested in the ${pkg.name} Meta Ads Package (${pkg.price}).`)}
                    >
                      Select {pkg.name}
                    </Button>
                  </CardFooter>
