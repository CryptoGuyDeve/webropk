...
export default function DigitalMarketingPageClient({ data }: { data: ServiceData | null }) {
  ...
  const DEFAULT_PACKAGES = [
    {
      ...
      features: ["Technical SEO Audit", "On-Page Optimization", "Keyword Research", "Google Business Profile Optimization", "Monthly Ranking Report"],
    },
    {
      name: "Expansion",
      price: "$800",
      description: "SEO + Retention.",
      features: ["Everything in Foundation", "Email Marketing Setup (Klaviyo/Mailchimp)", "4 Automated Flows (Welcome/Abandoned Cart)", "Weekly Blog Post (SEO Optimized)", "Backlink Building"],
      isPopular: true
    },
    {
      name: "Authority",
      ...
      features: ["Complete Content Strategy", "8 Blog Posts/Mo", "PR Outreach", "Advanced Email Segmentation", "Conversion Rate Optimization (CRO)", "Competitor Spy Reports"],
    }
  ];
          ...
          <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-20">
            ...
            >
              ...
              <p className="text-lg md:text-xl text-muted-foreground mb-8 leading-relaxed max-w-2xl mx-auto lg:mx-0">
                {data?.heroSubtitle || "SEO, Email, Content, and PR. We build the complete ecosystem that drives organic traffic and long-term customer value."}
              </p>
          ...
          <div className="grid lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
            <div className="grid lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
              {packages.map((pkg: any, i: number) => {
                ...
                const titleColor = isPopular ? "text-indigo-600" : "";
                ...
                return (
                  <CardWrapper key={i}>
                      ...
                      <ul className="space-y-3">
                        {pkg.features?.map((f: string, j: number) => (
                          <li key={j} className="flex items-center gap-2 text-sm text-foreground">
                            <CheckCircle2 className={`w-4 h-4 ${isPopular ? "text-indigo-500" : "text-gray-400"}`} /> {f}
                          </li>
                        ))}
                      </ul>
                    </CardContent>
                    <CardFooter>
                      <Button
                        className={`w-full ${isPopular ? "bg-indigo-500 hover:bg-indigo-600 text-white" : ""}`}
                        variant={isPopular ? "default" : "outline"}
                        onClick={() => openIntercomWithMessage(`Hi, I'm interested in the ${pkg.name} Digital Marketing Package (${pkg.price}).`)}
                      >
                        Select {pkg.name}
                      </Button>
                    </CardFooter>
