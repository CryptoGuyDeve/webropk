...
export default function GFXDesignPageClient({ data }: { data: ServiceData | null }) {
  ...
  const DEFAULT_PACKAGES = [
    {
      ...
      features: ["Professional Logo Design", "Secondary Mark/Icon", "Color Palette Hex Codes", "Typography Selection", "High-Res Exports (PNG/JPG)"],
    },
    {
      name: "Brand Kit",
      price: "$300",
      description: "Complete visual system.",
      features: ["Everything in Identity", "Vector Source Files (Ai/EPS)", "Social Media Profile Kit", "Business Card Design", "Brand Guidelines PDF", "3D Mockups"],
      isPopular: true
    },
    {
      name: "Full Stack",
      ...
      features: ["Everything in Brand Kit", "Web UI Kit (Homepage Design)", "Email Signature", "Letterhead & Envelope", "Social Media Templates (5)", "Priority Delivery"],
    }
  ];
          ...
          <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-20">
            ...
            >
              ...
              <p className="text-lg md:text-xl text-muted-foreground mb-8 leading-relaxed max-w-2xl mx-auto lg:mx-0">
                {data?.heroSubtitle || "Your brand's visual identity is your first impression. We craft logos, graphics, and UI deliverables that make you unforgettable."}
              </p>
          ...
          <div className="grid lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
            <div className="grid lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
              {packages.map((pkg: any, i: number) => {
                ...
                const titleColor = isPopular ? "text-rose-600" : "";
                ...
                return (
                  <CardWrapper key={i}>
                      ...
                      <ul className="space-y-3">
                        {pkg.features?.map((f: string, j: number) => (
                          <li key={j} className="flex items-center gap-2 text-sm text-foreground">
                            <CheckCircle2 className={`w-4 h-4 ${isPopular ? "text-rose-500" : "text-gray-400"}`} /> {f}
                          </li>
                        ))}
                      </ul>
                    </CardContent>
                    <CardFooter>
                      <Button
                        className={`w-full ${isPopular ? "bg-rose-500 hover:bg-rose-600 text-white" : ""}`}
                        variant={isPopular ? "default" : "outline"}
                        onClick={() => openIntercomWithMessage(`Hi, I'm interested in the ${pkg.name} GFX Design Package (${pkg.price}).`)}
                      >
                        Select {pkg.name}
                      </Button>
                    </CardFooter>
