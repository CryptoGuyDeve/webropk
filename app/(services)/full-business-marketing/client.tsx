...
export default function FullBusinessMarketingPageClient({ data }: { data: ServiceData | null }) {
  ...
  const DEFAULT_PACKAGES = [
    {
      ...
      features: ["Meta Ads Management", "Google Ads Management", "Social Media (3 posts/week)", "Monthly Strategy Call", "Live Dashboard"],
    },
    {
      name: "Scale Up",
      price: "$3500",
      description: "Omni-channel marketing.",
      features: ["Everything in Accelerator", "Full SEO Package", "Email Marketing Automation", "Content Creation (Blogs/Videos)", "Weekly Strategy Calls", "Slack Support Channel"],
      isPopular: true
    },
    {
      name: "Partner",
      ...
      features: ["Dedicated CMO", "Full Dev Support (Website edits)", "Influencer Marketing", "PR & Media Buying", "Daily Reporting", "Revenue Share Options"],
    }
  ];
          ...
          <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-20">
            ...
            >
              ...
              <p className="text-lg md:text-xl text-muted-foreground mb-8 leading-relaxed max-w-2xl mx-auto lg:mx-0">
                {data?.heroSubtitle || "Forget hiring freelancers or multiple agencies. We handle everything: Strategy, Ads, Content, SEO, and Development. One team, one goal."}
              </p>
          ...
          <div className="grid lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
            <div className="grid lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
              {packages.map((pkg: any, i: number) => {
                ...
                const titleColor = isPopular ? "text-foreground" : "";
                ...
                return (
                  <CardWrapper key={i}>
                      ...
                      <ul className="space-y-3">
                        {pkg.features?.map((f: string, j: number) => (
                          <li key={j} className="flex items-center gap-2 text-sm text-foreground">
                            <CheckCircle2 className={`w-4 h-4 ${isPopular ? "text-foreground" : "text-gray-400"}`} /> {f}
                          </li>
                        ))}
                      </ul>
                    </CardContent>
                    <CardFooter>
                      <Button
                        className={`w-full ${isPopular ? "bg-black hover:bg-zinc-800 text-white dark:bg-white dark:text-black dark:hover:bg-gray-200" : ""}`}
                        variant={isPopular ? "default" : "outline"}
                        onClick={() => openIntercomWithMessage(`Hi, I'm interested in the ${pkg.name} Business Marketing Package (${pkg.price}).`)}
                      >
                        Select {pkg.name}
                      </Button>
                    </CardFooter>
